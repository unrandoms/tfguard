...
def cli(arghandling=ArgHandling(), argparser=ArgumentParser(prog=__app_name__,
    ...
    parser.add_argument('--planfile', '-p', dest='plan_file', metavar='plan_file', action=ReadablePlan,
                        help='Plan output file generated by Terraform', required=False)
    parser.add_argument('--helm', dest='helm_chart_dir', metavar='chart_dir', type=str, nargs='?',
                        help='Path to a Helm chart directory; renders manifests into a pseudo Terraform plan',
                        required=False)
    parser.add_argument('--values', dest='helm_values_file', metavar='values_file', type=str, nargs='?',
                        help='Path to a Helm values YAML file used with --helm',
                        required=False)
    parser.add_argument('--output', '-o', dest='output_format', metavar='output_format', type=str,
                        default='default',
                        help='Output format: "default" (gherkin) or "github-actions" (workflow annotations)',
                        required=False)
    ...
    # --helm: render a Helm chart into a pseudo Terraform plan JSON.
    helm_chart_dir = getattr(args, 'helm_chart_dir', None)
    if helm_chart_dir:
        from terraform_compliance.common.helm_plan import write_helm_plan
        helm_values_file = getattr(args, 'helm_values_file', None)
        args.plan_file = write_helm_plan(helm_chart_dir, helm_values_file)
    elif not getattr(args, 'plan_file', None):
        parser.error('--planfile is required when --helm is not specified')
    ...
    output_format = getattr(args, 'output_format', 'default') or 'default'
    if output_format == 'github-actions':
        commands.append('--formatter=github_actions_formatter')
        load_module(os.path.join(os.path.dirname(__file__), "extensions/github_actions_formatter.py"))
        console_write('{} GitHub Actions annotation output enabled.'.format(Defaults().icon))
    elif args.silence is True:
